        ...
        for rdf_tree in all_metadata:
            ...
            for triple in creator_triples:
                ...
                if graph.value(subject=triple[2], predicate=RDF.type) == RDF.Bag:
                    for bag_triple in graph.triples((triple[2], None, None)):
                        ...
                        name_triple_ref = graph.value(
                            subject=bag_triple[2], predicate=self.CREATOR_NAME_REF
                        )
                        fullname = (
                            f"{graph.value(name_triple_ref, self.CREATOR_GIVEN_NAME_REF)} "
                            f"{graph.value(name_triple_ref, self.CREATOR_FAMILY_NAME_REF)}"
                        )
                ...
                # Else, we just get the name
                    ...
                    name_triple_ref = graph.value(
                        subject=triple[2], predicate=self.CREATOR_NAME_REF
                    )
                    fullname = (
                        f"{graph.value(name_triple_ref, self.CREATOR_GIVEN_NAME_REF)} "
                        f"{graph.value(name_triple_ref, self.CREATOR_FAMILY_NAME_REF)}"
